...
it('Test 0 wallets without price', () => {
  const tree = renderer
    .create(<AccountsStore.Context.Provider value={{ accounts: new Map() }}>
      <PricesStore.Context.Provider value={{ prices: new Map<Currency, number>() }}>
...
it('Test 1 wallets without price', () => {
  ...
  const tree = renderer
    .create(<AccountsStore.Context.Provider value={{ accounts: accounts }}>
      <PricesStore.Context.Provider value={{ prices: new Map<Currency, number>() }}>
        <Wallets navigation={null} />
      </PricesStore.Context.Provider>
    </AccountsStore.Context.Provider>)
...
it('Test click start', () => {
  ...
  const component = render(<AccountsStore.Context.Provider value={{ accounts: accounts }}>
    <PricesStore.Context.Provider value={{ prices: prices }}>
  ...
  expect(mockFn).toBeCalledWith("WalletDetails", {
    wallet: new Wallet(polkadotWallet?.name, polkadotWallet?.address, polkadotWallet?.currency, polkadotWallet?.balance, prices.get(Currency.Polkadot))
  });

  const kusamsWallet = accounts.get(Currency.Kusama)
  fireEvent.press(component.getByText(kusamsWallet?.name));
  expect(mockFn).toBeCalledWith("WalletDetails", {
    wallet: new Wallet(kusamsWallet?.name, kusamsWallet?.address, kusamsWallet?.currency, kusamsWallet?.balance, prices.get(Currency.Kusama))
  });
});
