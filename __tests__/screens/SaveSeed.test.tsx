...
it('Test save seed #1', () => {
  const seed = "run knee code wall merge impact teach grain slim quality patient curve"
  const tree = renderer
    .create(<SaveSeed navigation={null} route={{ params: { seed: seed.split(" ") } }} />)
    .toJSON();
  expect(tree).toMatchSnapshot();
});

it('Test save seed #1', () => {
  const seed = "staff select toddler junior robot own paper sniff glare drive stay census"
  const tree = renderer
    .create(<SaveSeed navigation={null} route={{ params: { seed: seed.split(" ") } }} />)
    .toJSON();
  expect(tree).toMatchSnapshot();
...
it('Test save click copy', () => {
  const seed = "staff select toddler junior robot own paper sniff glare drive stay census"
  const component = render(<SaveSeed navigation={null} route={{ params: { seed: seed.split(" ") } }} />)
...
it('Test save click next', () => {
  ...
  const seed = "staff select toddler junior robot own paper sniff glare drive stay census"
  const component = render(<SaveSeed navigation={{ navigate: mockFn }} route={{ params: { seed: seed.split(" ") } }} />)
