...
export const Backup = ({ navigation }: { navigation: any }) => {
    ...
    const backupSeed = async () => {
        ...
        navigation.navigate("SaveSeed", {
            seed: seed?.split(" "),
            onSuccess: async () => navigation.reset({
                index: 0,
                routes: [{ name: 'Home' }],
            }),
        })
    ...
    const MenuItems = [
        {
            img: <Image source={require('assets/img/google-drive.png')} style={{ width: 32, height: 32 }} />,
            title: 'Google disk',
        ...
        {
            img: <MaterialCommunityIcons name="content-copy" size={32} color="#888888" />,
            title: 'Backup seed',
            onClick: () => backupSeed()
        }
    ];
