...
it('Test view with empty txs', () => {
  ...
  expect(navigateMock).toBeCalledWith('Search', {
    currency: Currency.DOT,
  });

  fireEvent.press(component.getByTestId('receiveBtn'));
  expect(navigateMock).toBeCalledWith('Receive', {
    address: store.accounts.accounts[AccountType.Main][Currency.DOT].address,
    currency: Currency.DOT,
  });
});
