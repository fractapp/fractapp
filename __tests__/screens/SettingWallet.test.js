...
it('Test positive', () => {
  const tree = renderer.create(<SettingWallet navigation={null} route={{params:{seed: 'seed seed'}}}/>).toJSON();
  expect(tree).toMatchSnapshot();
});

it('Test click create new wallet', () => {
  const mockFn = jest.fn();
  const component = render(<SettingWallet navigation={{navigate: mockFn}} route={{params:{seed: 'seed seed'}}} />);
