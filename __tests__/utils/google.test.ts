...
it('Test safeSave fieldId == undefined', async () => {
    ...
    expect(GDrive.files.getId).toBeCalledWith(dir, ["root"])
    expect(GDrive.files.getId).toBeCalledWith(fileName, ["root", fieldIdDir])
    expect(GDrive.files.createFileMultipart).toBeCalledWith(file,
        "application/json",
        {
            parents: [fieldIdDir],
            name: fileName
...
it('Test getItems', async () => {
    ...
    for (let i = 0; i < 10; i++) {
        files.push({
            id: "id" + i,
            name: "name" + i,
            mimeType: i % 2 ? "application/vnd.google-apps.folder" : "application/json"
        })
    }
    files.push({
        id: "id invalid",
...
it('Test getFileBackup throw', async () => {
    const id = "testId"
